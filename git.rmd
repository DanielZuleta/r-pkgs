...
You can do many of these same things with other tools (like [subversion](https://subversion.apache.org) or [mercurial](http://mercurial.selenic.com)) and other websites (like [gitlab](https://about.gitlab.com) and [bitbucket](https://bitbucket.org)). Git is most useful in conjunction with GitHub, and vice versa, so I'll make no effort to distinguish between features that belong to Git and those that belong to GitHub. But I think Git + GitHub is the most user-friendly system (especially for new developers), not least because its popularity means that the answer or solution to every possible question or problem can be found on StackOverflow. 
...
1.  If needed, generate a SSH key. SSH keys allow you to securely 
    ...
    or use RStudio. Go to RStudio preferences (Tools > Global Options...), choose the Git/SVN panel, and 
...
To create a new repo:
  ...
  * In RStudio, go to project options, then to the Git/SVN panel. Change the 
    ...
    ```{r, echo = FALSE}
    oldbookdown::screenshot("screenshots/git-proj-config.png", dpi = 220)
    ```
...
Once Git has been initialised, you'll see two new components:
  ...
  * The __git pane__, at the top-right, shows you what files have 
    ...
    ```{r, echo = FALSE}
    oldbookdown::screenshot("screenshots/git-pane.png", dpi = 220)
    ```
  ...
  * The __git dropdown__ menu, found in the toolbar, includes Git and GitHub commands
    ...
    ```{r, echo = FALSE}
    oldbookdown::screenshot("screenshots/git-dropdown.png", dpi = 220)
    ```
...
* `r oldbookdown::screenshot("screenshots/git-modified.png", dpi = 220)`, 
  __Modified__. You've changed the contents of the file.

* `r oldbookdown::screenshot("screenshots/git-unknown.png", dpi = 220)`, 
  __Untracked__. You've added a new file that Git hasn't seen before.

* `r oldbookdown::screenshot("screenshots/git-deleted.png", dpi = 220)`, 
  __Deleted__. You've deleted a file.

You can get more details about modifications with a "diff", `r oldbookdown::screenshot("screenshots/git-diff.png", dpi = 220)`. This opens a new window showing the detailed **diff**erences:

```{r, echo = FALSE}
oldbookdown::screenshot("screenshots/git-diff-window.png", dpi = 220)
```
...
In RStudio, staging and committing are done in the same place, the commit window, which you can open by clicking `r oldbookdown::screenshot("screenshots/git-commit.png", dpi = 220)` or by pressing Ctrl + Alt + m.

```{r, echo = FALSE}
oldbookdown::screenshot("screenshots/git-commit-window.png", dpi = 220)
```
...
(Yes, this is exactly the same window you see when clicking `r oldbookdown::screenshot("screenshots/git-diff.png", dpi = 220)`!)
...
1.  __Open the commit window__ by clicking 
    `r oldbookdown::screenshot("screenshots/git-commit.png", dpi = 220)` or 
    pressing `Ctrl + Alt + m`.  
...
1.  __Select files__.  To stage (select) a single file for inclusion, tick its
    ...
    `r oldbookdown::screenshot("screenshots/git-stage.png", dpi = 220)`. 
    ...
    and you might see one of two new icons:
      ...
      * Added: `r oldbookdown::screenshot("screenshots/git-added.png", dpi = 220)`: 
        after staging an untracked file, Git now knows that you want to add it 
        to the repo.
      
      * Renamed: `r oldbookdown::screenshot("screenshots/git-renamed.png", dpi = 220)`:
    ...
    `r oldbookdown::screenshot("screenshots/git-modified-staged.png", dpi = 220)`.
...
```{r, echo = FALSE}
oldbookdown::screenshot("screenshots/git-ignore.png", dpi = 220)
```
...
*   To undo the changes you've just made, right click on the file in the Git 
    ...
    undo:`r oldbookdown::screenshot("screenshots/git-chunk.png", dpi = 220)`. You 
...
*   If you committed changes too early, you can modify the previous commit by 
    ...
    `r oldbookdown::screenshot("screenshots/git-commit-amend.png", dpi = 220)`. 
...
1.  Open the history window by clicking
    `r oldbookdown::screenshot("screenshots/git-history.png", dpi = 220)` in the 
    Git pane.
    
    ```{r, echo = FALSE}
    oldbookdown::screenshot("screenshots/git-history-window.png", dpi = 220)
    ```
...
1.  __Push__ your changes to GitHub by clicking 
    `r oldbookdown::screenshot("screenshots/git-push.png", dpi = 220)`. (This is 
...
Once you've connected your repo to GitHub, the Git pane will show you how many commits you have locally that are not on GitHub: `r oldbookdown::screenshot("screenshots/git-local-commits.png", dpi = 220)`. This message indicates that I have 1 commit locally (my branch) that is not on GitHub ("origin/master").
...
1.  You can comment on commits. To comment on the commit as a whole, use the 
    ...
    `r oldbookdown::screenshot("screenshots/github-comment-line.png", dpi = 220)`. 
...
* In the Git pane, you see a status like
  `r oldbookdown::screenshot("screenshots/git-commit-conflict.png", dpi = 220)`
...
To resolve a merge conflict, you need to open every file with the status `r oldbookdown::screenshot("screenshots/git-commit-conflict.png", dpi = 220)`. In each file, you'll find a conflict marker that looks like this:
